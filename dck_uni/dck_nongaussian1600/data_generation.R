...
setwd("/Users/beauniverse/Documents/GitHub/Deep-classifier-kriging-for-probabilistic-spatial-prediction-of-air-quality-index/dck_uni/dck_nongaussian1600")
mainDir <- "."
...
run_simulation <- function(sim) {
  x1 <- mvrnorm(1, rep(sample(seq(-2.5, 2.5, length.out = 100), 1), 1600), cov)
  x2 <- mvrnorm(1, rep(sample(seq(-2.5, 2.5, length.out = 100), 1), 1600), cov)
  x3 <- mvrnorm(1, rep(sample(seq(-2.5, 2.5, length.out = 100), 1), 1600), cov)
  x4 <- mvrnorm(1, rep(sample(seq(-2.5, 2.5, length.out = 100), 1), 1600), cov)
  x5 <- mvrnorm(1, rep(sample(seq(-2.5, 2.5, length.out = 100), 1), 1600), cov)
  
  mean_val <- x1^2 - x2^2 + x3^2 - x4^2 - x5^2 + 2*x1*x2 + 3*x2*x3 - 2*x3*x5 +
    10*x1*x4 + sin(x1)*x2*x3 + cos(x2)*x3*x5 + (x1*x2*x4*x5)
  
  simulation <- mvrnorm(1, rep(0, 1600), matern_cov1)
  ...
  write.csv(
    df,
    file = paste0(subDir, "sim1600_g", g, "_h", h, "_", sim, ".csv"),
    row.names = FALSE
  )
  return(NULL)
}
