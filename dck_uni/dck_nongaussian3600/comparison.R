...
for (C in C_list) {
  results_C <- foreach(sim = sim_list, .packages = c("VGAM","gridExtra"), .combine = "rbind") %dopar% {
    ...
    file_path_dnn <- paste0("synthetic_data_simulations/Results_DNN/sim3600_classified_g", 
                            g_val, "_h", h_val, "_", sim, ".csv")
    data_test <- read.csv(file_path_dnn, header = TRUE)
    
    file_path_train <- paste0("synthetic_data_simulations/training_data/sim3600_classified_g", 
