...
for (C in C_list) {
  results_C <- foreach(sim = sim_list, .packages = c("VGAM", "gridExtra"), .combine = "rbind") %dopar% {
    ...
    file_path_dnn <- paste0("Results_DNN/Tgh_nonGaussian_3600_classification_g", 
                            g_val, "_h", h_val, "_", sim, ".csv")
    data_test <- read.csv(file_path_dnn, header = TRUE)
    
    file_path_train <- paste0("synthetic_data_simulations/training_data/Tgh_nonGaussian_3600_classification_g", 
                              g_val, "_h", h_val, "_", sim, "train.csv")
    data_train <- read.csv(file_path_train, header = TRUE)
    
    # choose h
    sigma <- mad(data_train$tgh, constant = 1)  
    #sigma <- sd(data_train$tgh) 
    n <- nrow(data_train)   
    alpha = 1/3
    h <- C*sigma*n^(-alpha)/3
