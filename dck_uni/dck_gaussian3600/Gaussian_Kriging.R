...
pred_list_Gaussian <- foreach(sim = 1:num_sim, .packages = c("geoR"), .combine = "c") %dopar% {
  ...
  file_path <- paste0("synthetic_data_simulations/training_data/Tgh_nonGaussian_3600_classification_g", g_val,
                      "_h", h_val, "_", sim, "train.csv")
  data_train <- read.csv(file_path, header = TRUE)
  
  file_path <- paste0("synthetic_data_simulations/testing_data/Tgh_nonGaussian_3600_classification_g", g_val,
