...
pred_list_Gaussian <- foreach(sim = 1:num_sim, .packages = c("fields", "MASS", "geoR"), .combine = "c") %dopar% {
  ...
  file_path <- paste0("synthetic_data_simulations/training_data/2D_Gaussian_3600_projection_", sim, "train.csv")
  data_train <- read.csv(file_path, header = TRUE)
  
  file_path <- paste0("synthetic_data_simulations/testing_data/2D_Gaussian_3600_projection_", sim, "test.csv")
