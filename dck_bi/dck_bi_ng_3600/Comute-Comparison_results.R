...
for (C in C_grid) {
  results <- foreach(sim_num = 1:num_sim, .combine = rbind,
                     .packages = c("VGAM")) %dopar% {
                       ...
                       file_path = paste0("synthetic_data_simulations/testing_data/2D_nonGaussian_3600_projection_", sim_num, "test.csv")
                       data_test = read.csv(file_path, header = TRUE)
                       
                       file_path = paste0("Results_DNN/2D_nonGaussian_3600_predictions_", sim_num, ".csv")
                       data_test_dnn = read.csv(file_path, header = TRUE)
                       
                       file_path = paste0("synthetic_data_simulations/training_data/2D_nonGaussian_3600_projection_", sim_num, "train.csv")
