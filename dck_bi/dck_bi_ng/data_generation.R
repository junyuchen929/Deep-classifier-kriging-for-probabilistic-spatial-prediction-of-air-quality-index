...
library(MASS)
library(fields)
setwd("/Users/beauniverse/Documents/GitHub/Deep-classifier-kriging-for-probabilistic-spatial-prediction-of-air-quality-index/dck_bi/dck_bi_ng")
mainDir <- "."
...
for(i in 1:num_sim){
  simulation = mvrnorm(1,rep(0,2*1600),full_matern_cov)
  
  var1 = simulation[1:1600]
  var2 = simulation[1601:(2*1600)]
  g = 0.8
  h = 0.8
  ...
  write.csv(x = df,
            file = paste0("synthetic_data_simulations/2d_nongaussian_1600_",i,".csv"),
            row.names = F)
}
