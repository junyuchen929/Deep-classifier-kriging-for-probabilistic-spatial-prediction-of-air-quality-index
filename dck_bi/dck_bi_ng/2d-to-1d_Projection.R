...
for(sim in 1:num_sim){
  file_path <- paste0("synthetic_data_simulations/2d_nongaussian_1600_",sim,".csv")
  ...
  for(i in 1:dim(data)[1]){
    ...
    vec_distance = abs(data$var1[i] - b*data$var2[i] - a)/sqrt(1+b^2)
    index = order(vec_distance)[1]
    
    proj.var2 = (b[index]*data$var1[i] + data$var2[i] - a[index]*b[index])/(1 + b[index]^2)
    proj.var1 = b[index]*proj.var2 + a[index]
  ...
  write.csv(x = df_threshold2,
            file = paste0("synthetic_data_simulations/2D_nonGaussian_1600_projection_",sim,".csv"),
            row.names = F)
}
