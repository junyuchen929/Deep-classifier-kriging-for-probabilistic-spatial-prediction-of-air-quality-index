...
pred_list_all <- readRDS("pred_list_nonGaussian.rds")
results_all <- list()
...
for (C in C_grid) {
  results <- foreach(sim_num = 1:num_sim, .combine = rbind,
                     .packages = c("VGAM")) %dopar% {
                       ...
                       file_path = paste0("synthetic_data_simulations/testing_data/2D_nonGaussian_1600_projection_", sim_num, "test.csv")
                       data_test = read.csv(file_path, header = TRUE)
                       
                       file_path = paste0("Results_DNN/2D_nonGaussian_1600_predictions_", sim_num, ".csv")
                       data_test_dnn = read.csv(file_path, header = TRUE)
                       
                       file_path = paste0("synthetic_data_simulations/training_data/2D_nonGaussian_1600_projection_", sim_num, "train.csv")
                       ...
                       picp_dnn <- mean(data_test$var1 > lb_dnn & data_test$var1 < ub_dnn ,na.rm = TRUE)
                       
                       length_Gaussian <- mean(ub_Gaussian - lb_Gaussian)
                       length_dnn <- mean(ub_dnn - lb_dnn ,na.rm = TRUE)
